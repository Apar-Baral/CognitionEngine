...
def welcome_markup(*, project_root: str, initialized: bool, goal: str = "") -> str:
    lines = [
        ...
        "",
        "[bold]Copy & clipboard[/]",
        "  Clickable UI is default. For terminal selection, hold Shift while dragging in most terminals.",
        "  Terminal-only mode: [dim]CE_NATIVE_COPY=1[/] disables app mouse; use PgUp/Dn to scroll.",
        "  Backup file after each reply: [dim]~/.cognition/last_reply.txt[/]",
        "",
        "[bold]Scroll[/]",
        "  Mouse wheel when app mouse is on  ·  [dim]PgUp[/] / [dim]PgDn[/]",
        "",
        "[bold]Slash commands[/]",
